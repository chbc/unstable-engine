...
int main(int argc, char *argv[])
{
	...
	SDL_Log("### JANELAS: %d", state->num_windows);
	...
	SDL_Log("### CHAMANDO TEXTURA ###");
	uint32_t width, height;
	uint8_t bpp;
	void* data = loadTexture("media/crate.png", &width, &height, &bpp);

	SDL_Log("### GLM ###");
	glm::vec3 vector(1.0, 2.0, 3.0);
	SDL_Log("### vector.y: %.1f", vector.y);
